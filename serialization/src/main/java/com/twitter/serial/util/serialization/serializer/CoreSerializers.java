...
package com.twitter.serial.util.serialization.serializer;

import com.twitter.serial.util.internal.SerializableUtils;
import com.twitter.serial.util.object.ObjectUtils;
import com.twitter.serial.util.serialization.SerializationContext;
import com.twitter.serial.util.serialization.base.SerializableClass;
import com.twitter.serial.util.serialization.base.SerializationException;
import com.twitter.serial.util.serialization.base.SerializationUtils;
import com.twitter.serial.util.serialization.base.SerializerDefs;
...
public class CoreSerializers {
    ...
    private static final byte DEFAULT_COMPARATOR = 0;
    private static final byte INVERSE_COMPARATOR = 1;
    private static final byte TRIVIAL_COMPARATOR = 2;
    private static final byte STATELESS_OBJECT = 3;

    @NotNull
    public static <T> Serializer<Comparator<T>> getComparatorSerializer() {
        return new ObjectSerializer<Comparator<T>>() {
            @Override
            protected void serializeObject(@NotNull SerializationContext context,
                    @NotNull SerializerOutput output, @NotNull Comparator<T> comparator)
                    throws IOException {
                serializeComparator(context, output, comparator);
            }

            @NotNull
            @Override
            protected Comparator<T> deserializeObject(@NotNull SerializationContext context,
                    @NotNull SerializerInput input, int versionNumber)
                    throws IOException, ClassNotFoundException {
                return deserializeComparator(input);
            }
        };
    }
        ...
        return new ObjectSerializer<B>() {
            ...
            private void checkBaseClassSerializer(@NotNull SerializationContext context) {
                if (mNeedToCheckBaseClassSerializer && context.isDebug()) {
                    ...
                    for (int i = 1; i < len; ++i) {
                        final SerializableClass<B> subclass = ObjectUtils.cast(subclasses.get(i));
            ...
            protected B deserializeObject(@NotNull SerializationContext context,
                ...
                return ObjectUtils.cast(subclass.serializer.deserializeNotNull(context, input));
            }
        };
    }

    public static <T> void serializeComparator(@NotNull SerializationContext context,
            @NotNull SerializerOutput output,
            @NotNull Comparator<T> comparator)
            throws IOException {
        if (comparator == ObjectUtils.getComparator()) {
            output.writeByte(DEFAULT_COMPARATOR);
        } else if (comparator == ObjectUtils.getInverseComparator()) {
            output.writeByte(INVERSE_COMPARATOR);
        } else if (comparator == ObjectUtils.getTrivialComparator()) {
            output.writeByte(TRIVIAL_COMPARATOR);
        } else {
            output.writeByte(STATELESS_OBJECT);
            serializeStatelessObject(context, output, comparator);
        }
    }

    @NotNull
    public static <T> Comparator<T> deserializeComparator(@NotNull SerializerInput input)
            throws IOException, ClassNotFoundException {
        final byte comparatorType = input.readByte();
        switch (comparatorType) {
            case DEFAULT_COMPARATOR: {
                return ObjectUtils.cast(ObjectUtils.getComparator());
            }
            case INVERSE_COMPARATOR: {
                return ObjectUtils.cast(ObjectUtils.getInverseComparator());
            }
            case TRIVIAL_COMPARATOR: {
                return ObjectUtils.cast(ObjectUtils.getTrivialComparator());
            }
            case STATELESS_OBJECT: {
                return ObjectUtils.cast(deserializeStatelessObject(input));
            }
            default: {
