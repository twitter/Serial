...
import com.twitter.serial.util.internal.DebugClassDescriptor;
...
public class SerializationUtils {
    ...
    public static String dumpSerializedData(@NotNull SerializerInput input, int position, boolean includeValues) {
        final StringBuilder builder = new StringBuilder().append('{').append(StringUtils.lineSeparator());
        try {
            int objectNesting = 0;
            String indentation = "    ";
            ...
            while ((type = input.peekType()) != SerializerDefs.TYPE_EOF) {
                if (type == SerializerDefs.TYPE_END_OBJECT) {
                    ...
                    indentation = StringUtils.repeat("    ", objectNesting + 1);
                    input.readObjectEnd();
                        ...
                        case SerializerDefs.TYPE_STRING_UTF8: {
                            ...
                            if (includeValues) {
                                builder.append("String: \"")
                                        .append(string.replace(StringUtils.lineSeparator(), "\\n")).append('"');
                            } else {
